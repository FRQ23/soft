...
def _eval_regresion_uno(cromosoma, X_tr, Y_tr, atributos, salidas):
    yh      = predecir_regresion(cromosoma, X_tr, atributos, salidas)
    mse_val = MSE(yh, Y_tr)
    return 1.0 / (mse_val + 1e-6)
...
class AptitudRegresion(Aptitud):
    ...
    def evaluar_poblacion(self, poblacion, var):
        ...
        scores = Parallel(n_jobs=-1, prefer="threads")(
            delayed(_eval_regresion_uno)(c, X_tr, Y_tr, self.atributos, self.salidas)
            for c in poblacion
        )
...
if __name__ == "__main__":
    ...
    X_df  = df.drop(columns=drop_existentes).copy()
    ...
    # ── Feature engineering ──────────────────────────────────────────────────
    # 1. Seleccionar top 20 features más correlacionadas con SpO2
    TOP_K = 20
    selector = SelectKBest(f_regression, k=min(TOP_K, X_raw.shape[1]))
    selector.fit(X_raw, y_flat)
    X_top = selector.transform(X_raw)
    top_names = X_df.columns[selector.get_support()].tolist()
    print(f"  Top {TOP_K} features seleccionadas: {top_names}")

    # 2. Agregar features polinomiales de grado 2 (interacciones + cuadrados)
    poly = PolynomialFeatures(degree=2, include_bias=False)
    X_poly = poly.fit_transform(X_top)

    print(f"  Features originales : {X_raw.shape[1]}")
    print(f"  Features tras selección + poly: {X_poly.shape[1]}")
    ...
    ax.set_xlabel("SpO2 Real (%)",    fontsize=11)
    ax.set_ylabel("SpO2 Predicho (%)", fontsize=11)
    ax.set_title(f"SpO2 Real vs Predicho\nRMSE = {rmse_orig:.3f} pp",
    ...
    fig.suptitle(
        f"Regresion UCI — Prediccion de SpO2 con Algoritmo Genetico\n"
    ...
    # ── 8. Pairplot: SpO2 vs signos vitales clave ────────────────────────────
    print("\nGenerando pairplot …")
    ...
    grid.figure.suptitle(
        "UCI ICU — Signos Vitales vs Nivel de SpO2 (saturacion de oxigeno)",
